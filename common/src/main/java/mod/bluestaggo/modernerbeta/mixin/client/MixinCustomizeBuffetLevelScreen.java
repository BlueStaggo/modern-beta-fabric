...
public abstract class MixinCustomizeBuffetLevelScreen {
    ...
    private static String modifyTitle(String constant, @Local(argsOnly = true, ordinal = 0) Screen parent) {
        if (parent instanceof ModernBetaScreen) {
    ...
    @Redirect(
        ...
        at = @At(
            value = "FIELD",
            target = "Lnet/minecraft/client/gui/screen/CustomizeBuffetLevelScreen;BUFFET_BIOME_TEXT:Lnet/minecraft/text/Text;"
        )
    )
    private Text modifySubtitle() {
        if (this.parent instanceof ModernBetaScreen) {
